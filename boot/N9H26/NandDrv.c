	...
	if ((powerOnPageSize == 3) || (powerOnEcc == 3))	// Ignore power-on setting
	{
		...
		if (inIBR) {
			...
			if (pSM->nPageSize == NAND_PAGE_512B) {
				...
				outpw(REG_SMREAREA_CTL,
				      (inpw(REG_SMREAREA_CTL) & ~SMRE_REA128_EXT) | 16);
			} else if (pSM->nPageSize == NAND_PAGE_2KB) {
				outpw(REG_SMCSR, inpw(REG_SMCSR) & ~SMCR_BCH_TSEL);
				outpw(REG_SMCSR, inpw(REG_SMCSR) | BCH_T4);
				outpw(REG_SMCSR, (inpw(REG_SMCSR) & (~SMCR_PSIZE)) | (PSIZE_2K));
				outpw(REG_SMREAREA_CTL,
				      (inpw(REG_SMREAREA_CTL) & ~SMRE_REA128_EXT) | 64);
				...
				outpw(REG_SMREAREA_CTL,
				      (inpw(REG_SMREAREA_CTL) & ~SMRE_REA128_EXT) | 128);
				...
				outpw(REG_SMREAREA_CTL,
				      (inpw(REG_SMREAREA_CTL) & ~SMRE_REA128_EXT) | 376);
			}
		} else		// not in IBR area
		{
			...
			if (pSM->nPageSize == NAND_PAGE_512B) {
				...
				if (pSM->bIsNandECC8 == TRUE) {
					...
					outpw(REG_SMCSR, inpw(REG_SMCSR) | BCH_T8);
					outpw(REG_SMCSR,
					      (inpw(REG_SMCSR) & (~SMCR_PSIZE)) | (PSIZE_4K));
					outpw(REG_SMREAREA_CTL,
					      (inpw(REG_SMREAREA_CTL) & ~SMRE_REA128_EXT) | 128);
				...
				} else if (pSM->bIsNandECC24 == TRUE)	// for Micron MT29F16G08CBACA and MT29F32G08CBACA
				{
					...
					outpw(REG_SMCSR,
					      (inpw(REG_SMCSR) & (~SMCR_PSIZE)) | (PSIZE_4K));
					outpw(REG_SMREAREA_CTL,
					      (inpw(REG_SMREAREA_CTL) & ~SMRE_REA128_EXT) | 216);
				}
			} else if (pSM->nPageSize == NAND_PAGE_8KB) {
				outpw(REG_SMCSR, inpw(REG_SMCSR) & ~SMCR_BCH_TSEL);
				outpw(REG_SMCSR, inpw(REG_SMCSR) | BCH_T24);
...
int fmiSM_ReadID(FMI_SM_INFO_T * pSM)
{
	...
	case 0xd1:
		...
		if (((tempID[0] == 0xC2) && (tempID[1] == 0xF1) && (tempID[2] == 0x80)
		     && (tempID[3] == 0x1D)) || ((tempID[0] == 0xC2) && (tempID[1] == 0xF1)
						 && (tempID[2] == 0x80) && (tempID[3] == 0x95)
						 && (tempID[4] == 0x02))) {
	...
	case 0xda:		// 256M
		...
		if ((tempID[0] == 0xC2) && (tempID[1] == 0xDA) && (tempID[2] == 0x90)
		    && (tempID[3] == 0x95) && (tempID[4] == 0x06)) {
			// The first ID of this NAND is 0xC2 BUT it is NOT NAND ROM (read only)
	...
	case 0xdc:		// 512M
		...
		if ((tempID[0] == 0x2C) && (tempID[2] == 0x90) && (tempID[3] == 0xA6)
		    && (tempID[4] == 0x54)) {
		...
		if ((tempID[0] == 0x98) && (tempID[2] == 0x90) && (tempID[3] == 0x26)
		    && (tempID[4] == 0x76)) {
			pSM->uBlockPerFlash = 2047;	// block index with 0-base. = physical blocks - 1
			pSM->uPagePerBlock = 64;
			pSM->nPageSize = NAND_PAGE_4KB;
		...
		if ((tempID[0] == 0xC2) && (tempID[1] == 0xDC) && (tempID[2] == 0x90)
		    && (tempID[3] == 0x95) && (tempID[4] == 0x56)) {
			// The first ID of this NAND is 0xC2 BUT it is NOT NAND ROM (read only)
			// So, we MUST modify the configuration of it
			//      1. change pSM->bIsCheckECC to TRUE to enable ECC feature;
			pSM->bIsCheckECC = TRUE;
	...
	case 0xd3:		// 1024M
		...
		if ((tempID[0] == 0xEC) && (tempID[2] == 0x51) && (tempID[3] == 0x95)
		    && (tempID[4] == 0x58)) {
		...
		if ((tempID[0] == 0xC2) && (tempID[2] == 0xD1) && (tempID[3] == 0x95)
		    && (tempID[4] == 0x5A)) {
			// The first ID of this NAND is 0xC2 BUT it is NOT NAND ROM (read only)
			// So, we MUST change pSM->bIsCheckECC to TRUE to enable ECC feature.
			pSM->bIsCheckECC = TRUE;
	...
	default:
		...
		if ((tempID[0] == 0x2C) && (tempID[1] == 0x48) && (tempID[2] == 0x04)
		    && (tempID[3] == 0x4A) && (tempID[4] == 0xA5)) {
			pSM->uBlockPerFlash = 2047;	// block index with 0-base. = physical blocks - 1
			pSM->uPagePerBlock = 256;
			pSM->nPageSize = NAND_PAGE_4KB;
			pSM->uSectorPerBlock = pSM->nPageSize / 512 * pSM->uPagePerBlock;
		...
		else if ((tempID[0] == 0x2C) && (tempID[1] == 0x68) && (tempID[2] == 0x04)
			 && (tempID[3] == 0x4A) && (tempID[4] == 0xA9)) {
			pSM->uBlockPerFlash = 4095;	// block index with 0-base. = physical blocks - 1
			pSM->uPagePerBlock = 256;
			pSM->nPageSize = NAND_PAGE_4KB;
			pSM->uSectorPerBlock = pSM->nPageSize / 512 * pSM->uPagePerBlock;
		...
		else if ((tempID[0] == 0xC2) && (tempID[1] == 0xF0) && (tempID[2] == 0x80)
			 && (tempID[3] == 0x1D)) {
		...
		ERR_PRINTF("ERROR: SM ID not support!! [%02x][%02x][%02x][%02x][%02x]\n", tempID[0],
			   tempID[1], tempID[2], tempID[3], tempID[4]);
		return -1;
	}

	DBG_PRINTF("SM ID [%02x][%02x][%02x][%02x][%02x]\n", tempID[0], tempID[1], tempID[2],
		   tempID[3], tempID[4]);
	return 0;
}
...
static void fmiSM_CorrectData_BCH(uint8_t ucFieidIndex, uint8_t ucErrorCnt, uint8_t * pDAddr)
{
	...
	for (ii = 0; ii < ucErrorCnt; ii++) {
		...
		// for wrong parity code in redundancy area
			...
			*((uint8_t *) smra_index + (parity_len * (ucFieidIndex - 1)) +
			  uaAddr[ii]) ^= uaData[ii];
		}
				...
				for (jj = 0; jj < uLoop; jj++) {
					...
					for (ii = 1; ii < 5; ii++) {
						...
						if ((uStatus & ECCST_F1_STAT) == 0x01)	// correctable error in field (jj*4+ii)
						{
							...
							fmiSM_CorrectData_BCH(jj * 4 + ii,
									      uErrorCnt,
									      (uint8_t *) uDAddr);
							DBG_PRINTF
							    ("Warning: Field %d have %d BCH error. Corrected!!\n",
							     jj * 4 + ii, uErrorCnt);
							break;
						} else if (((uStatus & ECCST_F1_STAT) == 0x02) || ((uStatus & ECCST_F1_STAT) == 0x03))	// uncorrectable error or ECC error in 1st field
						{
							ERR_PRINTF
							    ("ERROR: Field %d encountered uncorrectable BCH error!!\n",
							     jj * 4 + ii);
							uError = 1;
							break;
						}
		...
		while (1) {
			if (inpw(REG_SMISR) & SMISR_ECC_FIELD_IF) {
				if (inpw(REG_SMCSR) & BCH_T4)	// BCH_ECC selected
				{
					...
					{
						uErrorCnt = uStatus >> 2;
						fmiSM_CorrectData_BCH(1, uErrorCnt,
								      (uint8_t *) uDAddr);
					...
					{
						DBG_PRINTF
						    ("SM uncorrectable error is encountered, %4x !!\n",
						     uStatus);
						uError = 1;
					}
				} else {
...
int sicSMInit()
{
	if (!bIsNandInit) {
		...
		if ((u8PowerOn & 0x6) != 0x6) {	/* With Power-On-Setting for NAND */
			...
			systemAreaSize =
			    (gu_fmiSM_PageSize * gu_fmiSM_PagePerBlock * 4) / (pSM0->nPageSize *
									       pSM0->uPagePerBlock);
...
int CheckBadBlockMark(uint32_t BlockNo)
{
	...
	if (pSM->nPageSize == NAND_PAGE_512B) {
		...
		if ((data512 == 0xFF) && (data517 == 0xFF))	// check byte 1 & 6
		{
			...
			if (status < 0) {
				sysprintf
				    ("ERROR: CheckBadBlockMark() read fail, for block %d, return 0x%x\n",
				     BlockNo, status);
				return 1;
			}
			data512 = inpw(REG_SMDATA) & 0xff;
