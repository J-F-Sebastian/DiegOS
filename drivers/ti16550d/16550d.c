...
static inline void write_register(const uint16_t com,
                                  enum uart_registers reg,
                                  uint8_t data)
{
    out_byte(com + reg, data);
}

static inline uint8_t read_register(const uint16_t com,
                                    enum uart_registers reg)
{
...
static int uart_start(unsigned unitno)
{
    ...
    if ((EOK != thread_io_wait_init(&wq_w)) ||
        (EOK != thread_io_wait_init(&wq_r))) {
        return (EPERM);
    }
