...
static void vga_rect_wm2(point_t a, point_t b, unsigned wide, unsigned color)
{
	...
	if (b.x - a.x < (int)(2 * wide))
		return;
	if (b.y - a.y < (int)(2 * wide))
		return;
	if (!wide)
		return;
...
static void vga_write_bgtext_wm2(point_t a, unsigned bg, unsigned c, const char *str)
{
	unsigned loop, i = 0;
	unsigned offset;
...
static void vga_draw_sprite(point_t a, uint8_t sprite[], unsigned W, unsigned H, unsigned color)
{
