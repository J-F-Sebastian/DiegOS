...
static inline int FAT_cluster_isReadable(uint16_t FATContent)
{
    return (FAT_cluster_isEOC(FATContent) ||
            FAT_cluster_isBAD(FATContent) ||
            FAT_cluster_isFREE(FATContent)) ? 0 : 1;
}
...
static int FAT_clear_cluster_chain(struct FATVolume *vol, struct FAT *entry)
{
    ...
    if (FAT_cluster_isFREE(N) ||
        (!entry->DIR_FileSize && !(entry->DIR_Attr & ATTR_DIRECTORY)))
        return 0;
...
int FAT_truncate_entry(struct FATVolume *vol, const char *entryname)
{
    ...
    const uint8_t NOT_ALLOWED = (ATTR_READ_ONLY | ATTR_SYSTEM | ATTR_DIRECTORY | ATTR_VOLUME_ID);
...
int FAT_delete_entry(struct FATVolume *vol, const char *entryname)
{
    ...
    return (FAT_write_DIR_entry(vol, &entry) ||
            FAT_clear_cluster_chain(vol, &entry.entry) ||
            FAT_write_FAT_table(vol));
}
             ...
             unsigned *readlen)
{
    ...
    *readlen = 0;
    ...
    if (FAT_update_timestamps(&search.entry, FAT_TS_READ) ||
        FAT_write_DIR_entry(vol, &search))
    {
        /* EIO */
        return -1;
    }
...
FAT_write_update_on_disk(int appended,
                         ...
                         struct FAT_direntry_search_result *direntry)
{
...
static int
FAT_write_get_next(uint16_t cluster,
                   struct FATVolume *vol,
                   uint16_t *newcluster)
{
