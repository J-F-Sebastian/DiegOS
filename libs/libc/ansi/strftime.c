 ...
 */
static char *s_prnt(char *s, size_t maxsize, const char *str, int width)
{
...
static char *u_prnt(char *s, size_t maxsize, unsigned val, int width)
{
	int c;
...
size_t strftime(char *s, size_t maxsize, const char *format, const struct tm *timeptr)
{
	...
	tzset();		/* for %Z conversion */
	firsts = s;
	while (maxsize > 0 && *format) {
		...
		switch (*format++) {
		case 'a':
			s = s_prnt(s, maxsize, time_days[timeptr->tm_wday], ABB_LEN);
			maxsize -= s - olds;
			break;
		case 'A':
			s = s_prnt(s, maxsize, time_days[timeptr->tm_wday], -1);
			maxsize -= s - olds;
			break;
		case 'b':
			s = s_prnt(s, maxsize, time_months[timeptr->tm_mon], ABB_LEN);
			maxsize -= s - olds;
			break;
		case 'B':
			s = s_prnt(s, maxsize, time_months[timeptr->tm_mon], -1);
			maxsize -= s - olds;
			break;
		case 'c':
		...
		case 'H':
			...
			break;
		case 'I':
			s = u_prnt(s, maxsize, (timeptr->tm_hour + 11) % 12 + 1, 2);
			maxsize -= s - olds;
			break;
		case 'j':
		...
		case 'M':
			...
			break;
		case 'p':
			s = s_prnt(s, maxsize, (timeptr->tm_hour < 12) ? "AM" : "PM", 2);
			maxsize -= s - olds;
			break;
		case 'S':
		...
		case 'Y':
			...
			break;
		case 'Z':
			s = s_prnt(s, maxsize, tzname[(timeptr->tm_isdst > 0)], -1);
			maxsize -= s - olds;
			break;
		case '%':
			*s++ = '%';
